...
class AirlineServletTest {
  ...
  void addOneFlightToAnAirline() throws IOException {
    ...
    Airline airline = servlet.getOrCreateAirline(airlineName.toUpperCase());
...
//    assertThat(flights, hasSize(0));
    ...
    void returnTextRepresentationOfAirline() throws IOException, ParserException, ParserConfigurationException, ParserException {
      ...
      HttpServletRequest request = mock(HttpServletRequest.class);
      when(request.getParameter(AirlineServlet.AIRLINE_NAME_PARAMETER)).thenReturn(airlineName.toUpperCase());
      ...
      assertThat(airline2.getName(), equalToIgnoringCase(airlineName));
      ...
      @Test
      void returnEmptyRepresentationOfAirline() throws IOException, ParserException, ParserConfigurationException {
        String airlineName = "Test Airlines";
        int flightNumber = 123;
        String src = "PDX";
        String dest = "SLC";
        ...
        HttpServletRequest request = mock(HttpServletRequest.class);
        when(request.getParameter(AirlineServlet.AIRLINE_NAME_PARAMETER)).thenReturn(airlineName.toUpperCase());
        when(request.getParameter(AirlineServlet.SRC_PARAMETER)).thenReturn(src.toUpperCase());
        when(request.getParameter(AirlineServlet.DEST_PARAMETER)).thenReturn(dest.toUpperCase());
        ...
        assertThat(airline2.getName(), equalTo(airlineName.toUpperCase()));
        Collection<Flight> flights = airline2.getFlights();
        assertThat(flights, hasSize(1));
        Flight flight = flights.iterator().next();
        assertThat(flight.getNumber(), equalTo(flightNumber));
      ...
      @Test
      void doGetWithoutBothCodes() throws IOException, ParserException, ParserConfigurationException {
          String airlineName = "Test Airlines";
          String src = "PDX";
          String dest = "SLC";
          AirlineServlet servlet = new AirlineServlet();
          ...
          servlet.doGet(request, response);
          verify(response).sendError(HttpServletResponse.SC_PRECONDITION_FAILED,"The required parameter \"dest\" is missing");


  }
      @Test
      void addFlightNullArgsSendsError() throws IOException {
        AirlineServlet servlet = new AirlineServlet();
        ...
        servlet.doPost(request, response);
        verify(response).sendError(HttpServletResponse.SC_PRECONDITION_FAILED, "The required parameter \"src\" is missing");
      }

  @Test
  void addFlightNullArgs2SendsError() throws IOException {
    AirlineServlet servlet = new AirlineServlet();
    ...
    when(request.getParameter(AirlineServlet.SRC_PARAMETER)).thenReturn(src);
    when(request.getParameter(AirlineServlet.DEST_PARAMETER)).thenReturn(dest);
    when(request.getParameter(AirlineServlet.DEPARTURE_PARAMETER)).thenReturn(null);
    when(request.getParameter(AirlineServlet.ARRIVAL_PARAMETER)).thenReturn(arrival);

    HttpServletResponse response = mock(HttpServletResponse.class);

    servlet.doPost(request, response);
    verify(response).sendError(HttpServletResponse.SC_PRECONDITION_FAILED, "The required parameter \"departure\" is missing");
  }
  @Test
  void addFlightNullArgs3SendsError() throws IOException {
    AirlineServlet servlet = new AirlineServlet();
    ...
    servlet.doPost(request, response);
    verify(response).sendError(HttpServletResponse.SC_PRECONDITION_FAILED, "The required parameter \"airline\" is missing");
  }
}
